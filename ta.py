...
def hhv(s, n):
    return s.rolling(n).max() #moments.rolling_max(s, n)
...
def ema(s, n, wilder=False):
    ...
    return s.rolling.ewm(span=span) #moments.ewma(s, span=span)
...
def aroon(s, n=25):
    up = 100 * s.rolling.rolling_apply(s.high, n + 1, lambda x: x.argmax()) / n
    dn = 100 * s.rolling.rolling_apply(s.low, n + 1, lambda x: x.argmin()) / n
...
def cci(s, n=20, c=0.015):
    ...
    mdev = s.rolling.rolling_apply(s, n, lambda x: np.fabs(x - x.mean()).mean())
...
def kst(s, r1=10, r2=15, r3=20, r4=30, n1=10, n2=10, n3=10, n4=15, nsig=9):
    ...
    s3 = s / s.shift(r3) - 1
    s4 = s / s.shift(r4) - 1
    rocma1 = s1.rolling(window=n1).mean()
    rocma2 = s2.rolling(window=n2).mean()
    rocma3 = s3.rolling(window=n3).mean()
    rocma4 = s4.rolling(window=n4).mean()

    # rocma1 = moments.rolling_mean(, n1)
    ...
    sig = kst.rolling(window=nsig).mean()
...
def auto_envelope(s, nema=22, nsmooth=100, ndev=2.7):
    ...
    csize = mdiff.ewm(span=nsmooth).std()*ndev
